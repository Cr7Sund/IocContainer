...
namespace IocContainer.Binder
{
    public class Injector : IInjector
    {
        ...
        private void failIf(bool condition, InjectionExceptionType errorCode)
        {
            failIf(condition, errorCode, null, null, null);
        }

        private void failIf(bool condition, InjectionExceptionType errorCode, Type t, object name)
        {
            failIf(condition, errorCode, t, name, null);
        }

        private void failIf(bool condition, InjectionExceptionType errorCode, Type t, object name, object target)
        {
            if (condition)
            {
                ...
                sb.Append($"\n\t\ttype: {t}");
                sb.Append($"\n\t\tname: {name}");
                throw new MyException(sb.ToString(), errorCode);
            }
        }
